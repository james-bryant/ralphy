...
| Loop Start | Start one eligible story from `Execution`. | The story moves into a queued or running state, the `Story Progress Dashboard` updates the current story plus pending/running/passed counts, live output begins, and the launcher uses the native Windows profile. |
...
| Loop Start | Start one eligible story from `Execution` while the WSL profile is selected. | The story moves into a queued or running state, the `Story Progress Dashboard` updates the current story plus pending/running/passed counts, live output begins, and the launcher runs through the saved WSL profile and mapped repository path. |
