...
40. In `Story Session`, confirm the card shows `Start Story Session` and `Pause`, and that `Pause` stays disabled until a story session is actively running.
...
| Loop Start | Choose `Story Implementation`, then use `Start Story Session` from `Execution`. | The next eligible story moves into a queued or running state, the `Story Progress Dashboard` updates the current story plus pending/running/passed counts, the `Run Output` card begins streaming live output in `Raw Output`, and the launcher uses the native Windows profile. |
...
| Loop Start | Choose `Story Implementation`, then use `Start Story Session` from `Execution` while the WSL profile is selected. | The next eligible story moves into a queued or running state, the `Story Progress Dashboard` updates the current story plus pending/running/passed counts, the `Run Output` card begins streaming live output in `Raw Output`, and the launcher runs through the saved WSL profile and mapped repository path. |
