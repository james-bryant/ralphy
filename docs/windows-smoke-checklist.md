...
40. In `Story Session`, confirm the card shows `Play` and `Pause`, that the session detail identifies any blocked stories that will be skipped, and that `Pause` stays disabled until a story session is actively running.
41. Close the app cleanly.
...
| Loop Start | Choose `Story Implementation`, then use `Play` from `Execution`. | `Play` starts with the next eligible story, skips blocked stories with a visible reason, retries one transient failure automatically, stops for review only after a second failure, continues automatically across later ready stories without another click when the retry passes, the `Story Progress Dashboard` updates the current story plus pending/running/passed counts, the `Run Output` card begins streaming live output in `Raw Output`, and the launcher uses the native Windows profile. |
| Pause | Request `Pause` while a story is running. | The session first shows `Pause requested`, the current step is allowed to finish, the UI then transitions to a fully paused state, and the next story does not start. |
...
| Run History | In `Execution > Run History`, review multiple persisted attempts for the same story and open the stored prompt, stdout, assistant summary, and artifact-summary JSON for both a passed and failed run. | The history list shows timestamps, branch, commit, preset, and result for each attempt, opening a history artifact loads its stored file content inside the viewer, and the same history remains available after closing and reopening the app. |
...
| Loop Start | Choose `Story Implementation`, then use `Play` from `Execution` while the WSL profile is selected. | `Play` starts with the next eligible story, skips blocked stories with a visible reason, retries one transient failure automatically, stops for review only after a second failure, continues automatically across later ready stories without another click when the retry passes, the `Story Progress Dashboard` updates the current story plus pending/running/passed counts, the `Run Output` card begins streaming live output in `Raw Output`, and the launcher runs through the saved WSL profile and mapped repository path. |
| Pause | Request `Pause` while a story is running through WSL. | The session first shows `Pause requested`, the current step is allowed to finish inside WSL, the UI reaches a fully paused state, and the next story does not start. |
...
| Run History | In `Execution > Run History`, review multiple persisted attempts for the same story and open the stored prompt, stdout, assistant summary, and artifact-summary JSON for both a passed and failed run. | The history list shows timestamps, branch, commit, preset, and result for each attempt, opening a history artifact loads its stored file content inside the viewer, and the same history remains available after closing and reopening the app under the saved WSL profile. |
